    ...
    >
      <el-table-column key="2" v-if="handleNum === 'many'" width="55">
        ...
        <template slot-scope="scope">
          <el-checkbox :value="checkedIds.indexOf(scope.row.spotId) > -1" @change="(v) => changeOne(v, scope.row)" />
        </template>
      </el-table-column>
      <el-table-column width="30" key="1" v-if="handleNum !== 'many'">
        <template slot-scope="scope">
          <el-radio :label="scope.row.spotId" v-model="templateRadio" @change.native="getTemplateRow(scope.row)"
            >&nbsp</el-radio
          >
        </template>
      </el-table-column>
      <el-table-column prop="spotId" label="ID" min-width="50" />
      <el-table-column label="商品图" width="80">
        <template slot-scope="scope">
          <div class="demo-image__preview line-heightOne">
            <el-image :src="scope.row.scenicSpot.image" :preview-src-list="[scope.row.scenicSpot.image]" />
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="scenicSpot.scenicName" label="商品名称" min-width="180" />
      <!-- <el-table-column prop="categoryName" label="商品分类" min-width="100" /> -->
      <el-table-column label="地址" min-width="100">
        <template slot-scope="scope">
          <div>{{ scope.row.scenicSpot.address }}</div>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    changeOne(v, order) {
      if (v) {
        const index = this.checkedIds.indexOf(order.spotId);
        if (index === -1) {
          this.checkedIds.push(order.spotId);
