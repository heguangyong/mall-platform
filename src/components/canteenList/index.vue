    ...
    >
      <el-table-column key="2" v-if="handleNum === 'many'" width="55">
        ...
        <template slot-scope="scope">
          <el-checkbox :value="checkedIds.indexOf(scope.row.storeId) > -1" @change="(v) => changeOne(v, scope.row)" />
        </template>
      </el-table-column>
      <el-table-column width="30" key="1" v-if="handleNum !== 'many'">
        <template slot-scope="scope">
          <el-radio :label="scope.row.storeId" v-model="templateRadio" @change.native="getTemplateRow(scope.row)"
            >&nbsp</el-radio
          >
        </template>
      </el-table-column>
      <el-table-column prop="storeId" label="ID" min-width="50" />
      <el-table-column label="商品图" width="80">
        <template slot-scope="scope">
          <div class="demo-image__preview line-heightOne">
            <el-image :src="scope.row.merchant.coverImage" :preview-src-list="[scope.row.merchant.coverImage]" />
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="merchant.name" label="商品名称" min-width="180" />
...
export default {
  ...
  methods: {
    ...
    changeOne(v, order) {
      if (v) {
        const index = this.checkedIds.indexOf(order.storeId);
        if (index === -1) {
          this.checkedIds.push(order.storeId);
    ...
    syncCheckedId(o) {
      const ids = this.tableData.data.map((v) => v.storeId);
      if (o) {
        this.tableData.data.forEach((item) => {
          const index = this.checkedIds.indexOf(item.storeId);
          if (index === -1) {
            this.checkedIds.push(item.storeId);
    ...
    handleSelectionChange(val) {
      const tables = [];
      val.map((item) => {
        tables.push({ src: item.image, id: item.storeId });
      });
        ...
        .then((res) => {
          ...
          this.tableData.data.forEach((item) => {
            this.checked.forEach((element) => {
              if (Number(item.storeId) === Number(element.storeId)) {
                this.$nextTick(() => {
