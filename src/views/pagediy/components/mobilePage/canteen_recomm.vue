          ...
          <div class="list-wrapper itemA" v-if="list.length > 0">
            ...
            >
              ...
              <div class="info">
                <div class="hd">
                  <div class="text">
                    <div class="title line2" :style="titleColor">
                      <span v-if="titleShow">{{ item.merchant.name }}</span>
                    </div>
                  </div>
                </div>
                ...
                <div class="tag-ppr">
                  <div class="tag" :style="tagColor" v-if="tagShow">{{item.merchant.keywords}}</div>
                  <div class="ppr" v-if="priceShow">¥{{item.merchant.balance}}/人</div>
                </div>
                <div class="address" v-if="addressShow">
                  <div class="addr">{{item.merchant.categoryName}} {{item.merchant.addressDetail}}</div>
                  <!-- <div class="distance">距离1.2km</div> -->
                </div>
              </div>
            </div>
          </div>
          <div class="list-wrapper itemA" v-else>
            <div class="item" :style="{ marginBottom: contentConfig + 'px' }">
              ...
              <div class="info">
                ...
                <div class="tag-ppr">
                  <div class="tag" :style="tagColor" v-if="tagShow">风味小吃</div>
                  <div class="ppr" v-if="priceShow">¥20/人</div>
                </div>
                <div class="address" v-if="addressShow">
                  <div class="addr">快餐简餐 江都区</div>
                  <div class="distance">距离1.2km</div>
                </div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    tagColor() {
      return [
        { color: this.configObj.tagColor.color[0].item },
        {
          background:
            this.configObj.tagColor.color[0].item[0] === '#'
              ? this.configObj.tagColor.color[0].item + '26'
              : this.configObj.tagColor.color[0].item.replace(
                  /rgba\((\d+,\s*\d+,\s*\d+,\s*)[\d\.]+\)/,
                  'rgba($10.15)',
                ),
        },
    ...
    return {
      ...
      defaultConfig: {
        ...
        titleFuConfig: {
          tabTitle: '文本标题',
          title: '副标题',
          val: '舌尖美味，餐厅精选，尽享地道佳肴！',
          place: '请输入副标题，限制24个字以内',
          max: 24,
          isShow: 1,
        },
        titleRightConfig: {
