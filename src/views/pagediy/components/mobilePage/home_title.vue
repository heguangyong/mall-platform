<template>
  <div v-if="configObj" class="title-box acea-row row-between row-middle" :style="boxStyle">
    <div class="acea-row row-bottom">
      <!-- <div :style="titleStyle">{{ titleTxt }}</div> -->
      <span style="color: #000;font-weight:bold;font-size:15px;">{{ titleTxt.slice(0, 2) }}</span>
    <span style="color: #2CCA83;font-weight: bold;font-size:15px;">{{ titleTxt.slice(2) }}</span>
      <div class="ml6" :style="titleFuStyle">{{ titleFuTxt }}</div>
    </div>
    <div v-if="!selectShow" class="more-box" :style="titleRightStyle">{{ titleRightTxt }}<i class="el-icon-caret-right"></i></div>
  </div>
</template>
...
.more-box{
  ...
  padding: 3px 5px;
  // height: 15px;
  background: rgba(44, 202, 131, 0.15);
  border-radius: 8px;
  // opacity: 0.15;
  font-size: rgba(44, 202, 131, 1);
}
</style>
