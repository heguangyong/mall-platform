...
// +----------------------------------------------------------------------
import { pagediyInfoApi, pagediySaveApi, pagediyUpdateApi } from '@/api/devise';
...
{
    "value": {  // 所有的组件配置
        "1744599209669000": {  // 组件的id
            "titleConfig": {  // 组件的配置（一般val就是配置的值）
                "val": "标题",
                "tabTitle": "文本标题",
                "max": 8,
                "place": "请输入标题，限制8个字以内",
    ...
    arraySort() {
      ...
      tempArr.push(custom);
      // tempArr.push(basis, marketing, tool,custom);
  ...
  onLog(evt) {
    ...
    if (evt.moved) {
      ...
      if (['search_box', 'nav_bar', 'home_comb'].includes(movedItem.name)) {
        this.$message.warning('该组件禁止拖拽');
        return;
      }
      ...
      this.rConfig = [JSON.parse(JSON.stringify(movedItem))];

      this.activeIndex = evt.moved.newIndex;
      this.$store.commit('mobildConfig/SETCONFIGNAME', movedItem.name);
      this.$store.commit('mobildConfig/defaultArraySort', evt.moved);
    }

    // 从左向右拖拽添加
    if (evt.added) {
      console.log('evt.added',evt.added)
      const addedItem = evt.added.element;
      let timestamp = new Date().getTime() * 1000;

      addedItem.num = timestamp;
      addedItem.id = 'id' + timestamp;

      this.activeConfigName = addedItem.name;
      this.mConfig[evt.added.newIndex] = JSON.parse(JSON.stringify(addedItem));

      this.rConfig = [JSON.parse(JSON.stringify(addedItem))];
      ...
      this.$store.commit('mobildConfig/SETCONFIGNAME', addedItem.name);
